...
import System.Directory (createDirectoryIfMissing, setCurrentDirectory, getCurrentDirectory, listDirectory)
  ...
  describe "SARA Stress Testing" $ do
    it "handles 1000 Markdown files efficiently" $ do
      withSystemTempDirectory "sara-stress" $ \tmpDir -> do
        ...
        -- Verify the real engine actually produced all 1000 rendered
        -- files, not just that it exited without throwing. Output
        -- extension is now .html by default (SlugRoute), matching
        -- every mainstream SSG's default and fixing the previous
        -- .md-preserving behavior documented (and now removed) here.
        renderedFiles <- listDirectory (siteDir </> "posts")
        length renderedFiles `shouldBe` 1000

        -- Spot-check one output file's actual rendered content, so a
        -- build that silently emits 1000 empty or malformed files
        -- would be caught, not just "didn't crash."
        sample <- TIO.readFile (siteDir </> "posts" </> "post-1.html")
        sample `shouldSatisfy` T.isInfixOf "Body 1"
