...
generateSiteHs = T.unlines
  ...
  , "  -- behaves -- see the SARA.Content.Drafts module. Filtering happens"
  , "  -- here, before this list feeds the search index/sitemap/feed below,"
  , "  -- so an unpublished post is never indexed or syndicated either."
...
searchJs = T.unlines
  ...
  , "    div.addEventListener('click', () => { location.href = res.seUrl; });"
  , ""
  , "    const h3 = document.createElement('h3');"
  , "    h3.textContent = res.seTitle;"
  , "    const p = document.createElement('p');"
  , "    p.textContent = res.seUrl;"
  , ""
  , "    div.appendChild(h3);"
  , "    div.appendChild(p);"
