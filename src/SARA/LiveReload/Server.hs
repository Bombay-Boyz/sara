...
--   Prunes clients whose send failed, the same way 'broadcastMessage'
--   does — a disconnected client should not silently accumulate in the
--   map forever just because it happened to be filtered by path rather
...
broadcastToPath clientsMVar path msg = modifyMVar_ clientsMVar $ \clientMap -> do
  ...
  let failedTargets = [c | (c, False) <- zip targetClients results]
  pure $ Map.filterWithKey (\client _ -> client `notElem` failedTargets) clientMap
  where
    sendMessage payload client = handle (\(_ :: SomeException) -> pure False) $ do
      WS.sendDataMessage (mcConn client) (WS.Text payload Nothing)
      pure True
...
wsApp clientsMVar pending = do
  ...
  case Aeson.decode (BSL.fromStrict $ TE.encodeUtf8 msg) of
    Just (Aeson.Object obj) | Just (Aeson.String path) <- Map.lookup "path" (toDataKeyMap obj) -> do
       -- A fresh, globally-unique id per connection via 'Data.Unique'
       -- (base's dedicated supply of unique values), rather than a
       -- module-level mutable counter — this needs no
       -- 'unsafePerformIO'\/'NOINLINE' escape hatch (banned by 1.7\/1.8)
       -- because the uniqueness is provided by the runtime, not by a
       -- hand-rolled global 'IORef'.
