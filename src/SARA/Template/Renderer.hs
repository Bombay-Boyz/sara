...
addTemplateOracle = void $ addOracle $ \(TemplateOracle path) -> do
  ...
  case auditErrors of
    [] -> do
      ...
      pure path
    errs -> do
      -- Same justified Shake-Action-boundary exception as
      -- 'SARA.Internal.Hash.addLQIPOracle' — see 'SaraBuildException'.
      liftIO $ throwIO (SaraBuildException (T.pack ("Security audit failed for template " ++ path ++ ": " ++ show errs)))
